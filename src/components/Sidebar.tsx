...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, onTabChange, isOpen, onClose }) => {
  const menuItems = [
    ...
    { id: AnalysisTab.TRACK_RECORD, icon: 'fa-clipboard-check', label: 'Track Record' },
  ...
  return (
    <>
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 transition-transform duration-300 w-56 bg-slate-950 border-r border-white/5 flex flex-col h-full`}>
      <div className="p-5">
        <div className="flex items-center justify-between gap-2 mb-6">
          ...
          <button
            onClick={onClose}
            className="lg:hidden p-1.5 text-slate-500 hover:text-white hover:bg-white/5 rounded-lg transition-colors"
